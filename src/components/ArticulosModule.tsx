...
import {
  ...
  ArrowUpDown, Filter, Package, Save
} from 'lucide-react'
...
export default function ArticulosModule() {
  ...
  const fetchArticulos = useCallback(async () => {
    setLoading(true)
    let query = supabase.from('articulos').select('*').eq('activo', true)
  ...
  useEffect(() => {
    fetchArticulos()
    const channel = supabase.channel('articulos-changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'articulos' }, () => fetchArticulos())
      .subscribe()
  ...
  async function saveEdit() {
    ...
    await supabase.from('articulos').update({
      proveedor: editData.proveedor, codigo: editData.codigo, descripcion: editData.descripcion,
      cantidad: editData.cantidad, costo_unitario: editData.costo_unitario,
      precio_comparativo: editData.precio_comparativo, precio_venta: editData.precio_venta, nota: editData.nota,
    }).eq('id', editingId)
  ...
  async function addArticulo() {
    ...
    setShowAdd(false)
    setNewArticulo({ proveedor: '', codigo: '', descripcion: '', cantidad: 0, costo_unitario: 0, precio_comparativo: 0, precio_venta: 0, nota: '' })
  }
    ...
    <div className="p-4 lg:p-6 max-w-[1600px] mx-auto bg-botanical min-h-full">
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          ...
          <div className={cn("rounded-xl px-4 py-3 border", stockBajo > 0 ? "bg-red-50 border-red-100" : "bg-emerald-50 border-emerald-100")}>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <p className={cn("text-2xl font-semibold", stockBajo > 0 ? "text-red-600" : "text-emerald-600")}>{stockBajo}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Buscar por descripción, código o proveedor..."
              value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pl-9 pr-4 py-2.5 rounded-lg border border-gray-200 bg-white text-sm
                focus:outline-none focus:ring-2 focus:ring-kira-400/30 focus:border-kira-400 placeholder:text-gray-400" />
          </div>
          <div className="flex gap-2">
            <button onClick={() => setShowFilters(!showFilters)}
              className={cn("px-3 py-2.5 rounded-lg border text-sm flex items-center gap-2 transition-colors",
                showFilters || proveedorFilter ? "bg-kira-50 border-kira-200 text-kira-700" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50")}>
        ...
        {showAdd && (
          <div className="mb-4 p-4 bg-white rounded-xl border border-kira-100 animate-fade-in">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <input placeholder="Proveedor" value={newArticulo.proveedor}
              ...
              <input placeholder="Código" value={newArticulo.codigo}
                onChange={e => setNewArticulo(p => ({ ...p, codigo: e.target.value }))}
                className="col-span-1 px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-kira-400/30" />
              <input placeholder="Descripción *" value={newArticulo.descripcion}
              ...
              <input type="number" placeholder="Cantidad" value={newArticulo.cantidad || ''}
              ...
              <input type="number" placeholder="Costo unitario" value={newArticulo.costo_unitario || ''}
                onChange={e => setNewArticulo(p => ({ ...p, costo_unitario: Number(e.target.value) }))}
                className="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-kira-400/30" />
              <input type="number" placeholder="Precio comparativo" value={newArticulo.precio_comparativo || ''}
                onChange={e => setNewArticulo(p => ({ ...p, precio_comparativo: Number(e.target.value) }))}
                className="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-kira-400/30" />
              <input type="number" placeholder="Precio de venta" value={newArticulo.precio_venta || ''}
                onChange={e => setNewArticulo(p => ({ ...p, precio_venta: Number(e.target.value) }))}
                className="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-kira-400/30" />
              <input placeholder="Nota" value={newArticulo.nota}
              ...
              <button onClick={addArticulo} disabled={!newArticulo.descripcion}
                className="px-4 py-2 rounded-lg bg-kira-500 text-white text-sm font-medium hover:bg-kira-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
                ...
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  {[
                    ...
                    <th key={col.key} onClick={() => handleSort(col.key as keyof Articulo)}
                      className={cn("px-3 py-2.5 text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:text-gray-700 select-none whitespace-nowrap",
                        (col as any).align === 'right' ? 'text-right' : 'text-left')}>
                      <span className="inline-flex items-center gap-1">
                        {col.label}
                        {sortField === col.key && <ArrowUpDown size={11} className="text-kira-500" />}
                      </span>
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-gray-50">
                {loading ? (
                  <tr><td colSpan={8} className="px-4 py-12 text-center text-gray-400">Cargando artículos...</td></tr>
                ) : articulos.length === 0 ? (
                  <tr><td colSpan={8} className="px-4 py-12 text-center text-gray-400">
                    <Package size={32} className="mx-auto mb-2 text-gray-300" />No se encontraron artículos
                  </td></tr>
                ) : articulos.map((a) => {
                  const isEditing = editingId === a.id
                  const margenPct = a.costo_unitario > 0 ? ((a.precio_venta - a.costo_unitario) / a.costo_unitario * 100).toFixed(0) : '—'
                  return (
                    <tr key={a.id} className={cn("hover:bg-kira-50/30 transition-colors", a.cantidad <= 0 && "bg-red-50/40")}>
                      <td className="px-3 py-2">
                        {isEditing ? <input value={editData.proveedor ?? ''} onChange={e => setEditData(d => ({ ...d, proveedor: e.target.value }))}
                      ...
                      <td className="px-3 py-2">
                        {isEditing ? <input value={editData.codigo ?? ''} onChange={e => setEditData(d => ({ ...d, codigo: e.target.value }))}
                      ...
                      <td className="px-3 py-2 max-w-[240px]">
                        {isEditing ? <input value={editData.descripcion ?? ''} onChange={e => setEditData(d => ({ ...d, descripcion: e.target.value }))}
                          className="w-full px-2 py-1 text-sm border rounded focus:outline-none focus:ring-1 focus:ring-kira-400" />
                          : <span className="font-medium text-gray-800 truncate block">{a.descripcion}</span>}
                      </td>
                      <td className="px-3 py-2 text-right">
                        {isEditing ? <input type="number" value={editData.cantidad ?? 0} onChange={e => setEditData(d => ({ ...d, cantidad: Number(e.target.value) }))}
                          className="w-16 px-2 py-1 text-sm border rounded text-right focus:outline-none focus:ring-1 focus:ring-kira-400" />
                          : <span className={cn("inline-flex items-center justify-center min-w-[28px] px-1.5 py-0.5 rounded-md text-xs font-semibold",
                            a.cantidad <= 0 ? "bg-red-100 text-red-700" : a.cantidad <= 2 ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700")}>
                            {a.cantidad}</span>}
                      </td>
                      <td className="px-3 py-2 text-right">
                        {isEditing ? <input type="number" value={editData.costo_unitario ?? 0} onChange={e => setEditData(d => ({ ...d, costo_unitario: Number(e.target.value) }))}
                      ...
                      <td className="px-3 py-2 text-right">
                        {isEditing ? <input type="number" value={editData.precio_venta ?? 0} onChange={e => setEditData(d => ({ ...d, precio_venta: Number(e.target.value) }))}
                          className="w-24 px-2 py-1 text-sm border rounded text-right focus:outline-none focus:ring-1 focus:ring-kira-400" />
                          : <div>
                            <span className="font-semibold text-gray-800">{formatCurrency(a.precio_venta)}</span>
                            <span className={cn("block text-[10px]",
                              Number(margenPct) < 0 ? "text-red-500" : Number(margenPct) < 30 ? "text-amber-500" : "text-emerald-500")}>
                              {margenPct !== '—' ? `+${margenPct}%` : ''}</span>
                          </div>}
                      </td>
                      <td className="px-3 py-2 max-w-[160px]">
                        {isEditing ? <input value={editData.nota ?? ''} onChange={e => setEditData(d => ({ ...d, nota: e.target.value }))}
                          className="w-full px-2 py-1 text-sm border rounded focus:outline-none focus:ring-1 focus:ring-kira-400" />
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center justify-end gap-1">
                          {isEditing ? (<>
                            <button onClick={saveEdit} className="p-1.5 rounded-md hover:bg-emerald-50 text-emerald-600"><Check size={14} /></button>
                            <button onClick={() => { setEditingId(null); setEditData({}) }} className="p-1.5 rounded-md hover:bg-gray-100 text-gray-400"><X size={14} /></button>
                          </>) : (<>
                            <button onClick={() => startEdit(a)} className="p-1.5 rounded-md hover:bg-gray-100 text-gray-400 hover:text-gray-600"><Pencil size={13} /></button>
                            <button onClick={() => deleteArticulo(a.id)} className="p-1.5 rounded-md hover:bg-red-50 text-gray-400 hover:text-red-500"><Trash2 size={13} /></button>
                          </>)}
                        </div>
                      </td>
                    </tr>
