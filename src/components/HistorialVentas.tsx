...
import {
  ...
  Calendar, TrendingUp, XCircle, Package
} from 'lucide-react'
...
export default function HistorialVentas() {
    ...
    const { data } = await supabase
      ...
      .select(`
        *,
        venta_items (
          *,
          articulo:articulos ( descripcion, codigo )
        )
      `)
    ...
    try {
      ...
      for (const item of venta.venta_items) {
        ...
        if (art) {
          await supabase
            .from('articulos')
            .update({ cantidad: art.cantidad + item.cantidad })
            .eq('id', item.articulo_id)
        }
      }
      ...
      const { data: bill } = await supabase
        .from('billetera')
        .select('*')
        .eq('tipo', venta.metodo_pago)
        .single()

      if (bill) {
        await supabase
          .from('billetera')
          .update({ saldo: bill.saldo - venta.total })
          .eq('id', bill.id)
      }
  ...
  const ventasFiltradas = filtro === 'todas'
    ? ventas
    : ventas.filter(v => v.metodo_pago === filtro)
        ...
        {(['todas', 'efectivo', 'transferencia'] as const).map(f => (
          <button
            ...
            className={cn(
              ...
              filtro === f
                ? "bg-kira-100 text-kira-700 border border-kira-200"
          ...
          {Object.entries(ventasPorFecha).map(([fecha, ventasDelDia]) => (
            <div key={fecha}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-xs font-medium text-gray-400 uppercase tracking-wider capitalize">
                  {fecha}
                </h3>
                    ...
                    >
                      ...
                      >
                        <div className={cn(
                          ...
                          isAnulada ? "bg-red-100" :
                          venta.metodo_pago === 'efectivo' ? "bg-emerald-100" : "bg-blue-100"
                        )}>
                          {isAnulada ? (
                        ...
                        <span className={cn(
                          ...
                          isAnulada ? "text-red-400 line-through" : "text-gray-800"
                        )}>
