...
import { supabase, Proveedor, Comprobante, OrdenPago, Gasto, Articulo, COMPROBANTE_LABELS, COMPROBANTES_DEUDA, COMPROBANTES_CREDITO, COMPROBANTES_CON_IVA, TipoComprobante } from '@/lib/supabase'
import { formatCurrency, cn } from '@/lib/utils'
import { Search, Plus, Trash2, X, Check, ChevronRight, ArrowLeft, Building2, Zap, FileText, CreditCard, Banknote, AlertTriangle, Package, Save, DollarSign, Download, CheckSquare, Square } from 'lucide-react'
...
export default function ProveedoresModule() {
  ...
  async function fetchDetalle(prov: Proveedor) {
    const [comp, op, ga] = await Promise.all([
    ...
    if (prov.tipo === 'mercaderia') {
      ...
      if (pa && pa.length > 0) {
        const { data: arts } = await supabase.from('articulos').select('*').in('id', pa.map(x => x.articulo_id)).eq('activo', true).order('descripcion')
  ...
  function openDetalle(prov: Proveedor) {
    setProvActivo(prov); setVista('detalle'); setTab('cc'); setSelectedComps(new Set()); setOcItems({})
  ...
  async function saveProv() {
    const d = { nombre: formProv.nombre, tipo: formProv.tipo, cuit: formProv.cuit||null, direccion: formProv.direccion||null, localidad: formProv.localidad||null, provincia: formProv.provincia||null,
      ...
      frecuencia_pago: formProv.frecuencia_pago||null, proximo_vencimiento: formProv.proximo_vencimiento||null }
    if (editingProv && provActivo) {
      await supabase.from('proveedores').update(d).eq('id', provActivo.id)
      setEditingProv(false)
      const { data: u } = await supabase.from('proveedores').select('*').eq('id', provActivo.id).single()
      if (u) setProvActivo(u)
    } else {
      await supabase.from('proveedores').insert([d])
      setShowAdd(false)
  ...
  async function saveComprobante() {
    ...
    const sub = parseFloat(compForm.subtotal)||0, i21 = tieneIva?(parseFloat(compForm.iva_21)||0):0, i105 = tieneIva?(parseFloat(compForm.iva_105)||0):0
    const i27 = tieneIva?(parseFloat(compForm.iva_27)||0):0, perc = tieneIva?(parseFloat(compForm.percepciones)||0):0, otros = tieneIva?(parseFloat(compForm.otros_impuestos)||0):0
    ...
    await supabase.from('comprobantes').insert([{
      proveedor_id: provActivo.id, tipo_comprobante: compForm.tipo_comprobante, numero: compForm.numero||null,
      fecha: compForm.fecha, fecha_vencimiento: compForm.fecha_vencimiento||null,
      subtotal: sub, iva_21: i21, iva_105: i105, iva_27: i27, percepciones: perc, otros_impuestos: otros, total,
      descripcion: compForm.descripcion||null, saldo_pendiente: esCredito?0:total, estado: esCredito?'pagado':'pendiente' }])
  ...
  async function saveOrdenPago() {
    ...
    if (monto <= 0) return
    const compIds = Array.from(selectedComps)
    const esACuenta = compIds.length === 0

    // Create one OP per selected comprobante, or one a cuenta
    if (esACuenta) {
      await supabase.from('ordenes_pago').insert([{ proveedor_id: provActivo.id, metodo_pago: opForm.metodo_pago, monto, comprobante_id: null, es_a_cuenta: true, nota: opForm.nota||null }])
    } else {
      // Distribute monto across selected comprobantes
      let remaining = monto
      for (const cid of compIds) {
        const comp = comprobantes.find(c=>c.id===cid)
        if (!comp || remaining <= 0) continue
        const aplicar = Math.min(remaining, comp.saldo_pendiente)
        ...
        const nuevoSaldo = Math.max(0, comp.saldo_pendiente - aplicar)
        await supabase.from('comprobantes').update({ saldo_pendiente: nuevoSaldo, estado: nuevoSaldo<=0?'pagado':'parcial' }).eq('id', cid)
        remaining -= aplicar
      }
      // If there's remaining, create a cuenta
      if (remaining > 0) {
        await supabase.from('ordenes_pago').insert([{ proveedor_id: provActivo.id, metodo_pago: opForm.metodo_pago, monto: remaining, comprobante_id: null, es_a_cuenta: true, nota: 'Sobrante de pago' }])
      }
    }
    ...
    setShowOPForm(false); setOpForm({ metodo_pago:'transferencia',monto:'',nota:'' }); setSelectedComps(new Set())
    fetchDetalle(provActivo)
  }
  ...
  async function saveGasto() {
    ...
    await supabase.from('gastos').insert([{ descripcion: gastoForm.descripcion, monto, metodo_pago: gastoForm.metodo_pago, fecha: gastoForm.fecha,
      tiene_comprobante: gastoForm.tiene_comprobante, tipo_comprobante: gastoForm.tipo_comprobante||null, numero_comprobante: gastoForm.numero_comprobante||null, nota: gastoForm.nota||null }])
  ...
  function toggleCompSelect(id: number) {
    setSelectedComps(prev => { const n = new Set(prev); if (n.has(id)) n.delete(id); else n.add(id); return n })
  }

  const selectedTotal = useMemo(() => {
    return comprobantes.filter(c => selectedComps.has(c.id)).reduce((s,c) => s + c.saldo_pendiente, 0)
  }, [selectedComps, comprobantes])

  function openPaySelected() {
    setShowOPForm(true)
    setOpForm({ metodo_pago: 'transferencia', monto: selectedTotal.toString(), nota: '' })
  }

  // OC: set quantity
  function setOcQty(artId: number, qty: number) {
    setOcItems(prev => { const n = {...prev}; if (qty <= 0) delete n[artId]; else n[artId] = qty; return n })
  }

  const ocTotal = useMemo(() => {
    return Object.entries(ocItems).reduce((s, [id, qty]) => {
      const art = articulosProv.find(a => a.id === parseInt(id))
      return s + (art ? art.costo_unitario * qty : 0)
    }, 0)
  }, [ocItems, articulosProv])

  const ocIva = ocTotal * 0.21

  async function generarOCPdf() {
    if (!provActivo || Object.keys(ocItems).length === 0) return
    setGeneratingPdf(true)

    // Save OC to DB
    const ocNum = `OC-${Date.now().toString().slice(-6)}`
    const { data: oc } = await supabase.from('ordenes_compra').insert([{
      proveedor_id: provActivo.id, numero: ocNum, subtotal: ocTotal, iva: ocIva, total: ocTotal + ocIva, estado: 'enviada'
    }]).select().single()

    if (oc) {
      const items = Object.entries(ocItems).map(([id, qty]) => {
        const art = articulosProv.find(a => a.id === parseInt(id))!
        return { orden_compra_id: oc.id, articulo_id: parseInt(id), cantidad: qty, precio_unitario: art.costo_unitario, subtotal: art.costo_unitario * qty }
      })
      await supabase.from('orden_compra_items').insert(items)
    }

    // Generate PDF client-side
    const itemsList = Object.entries(ocItems).map(([id, qty]) => {
      const art = articulosProv.find(a => a.id === parseInt(id))!
      return { desc: art.descripcion, codigo: art.codigo, qty, precio: art.costo_unitario, sub: art.costo_unitario * qty }
    })

    // Build HTML for PDF
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
      body{font-family:Arial,sans-serif;margin:40px;color:#333}
      .header{display:flex;justify-content:space-between;border-bottom:2px solid #d78133;padding-bottom:15px;margin-bottom:20px}
      .logo h1{margin:0;color:#d78133;font-size:22px} .logo p{margin:2px 0;font-size:11px;color:#666}
      .oc-info{text-align:right} .oc-info h2{margin:0;font-size:16px} .oc-info p{margin:2px 0;font-size:11px;color:#666}
      .proveedor{background:#f9f5f0;padding:12px;border-radius:6px;margin-bottom:20px;font-size:12px}
      .proveedor strong{color:#d78133}
      table{width:100%;border-collapse:collapse;font-size:12px}
      th{background:#1c1917;color:white;padding:8px 10px;text-align:left}
      ...
      .totals .total{font-size:16px;font-weight:bold;color:#d78133;border-top:2px solid #d78133;padding-top:8px;margin-top:4px}
      ...
      <div class="header">
        <div class="logo">
          <h1>Kira Home</h1>
          <p>Bazar & Deco</p>
          <p>14 de Julio 3915</p>
          <p>Bahía Blanca, Buenos Aires</p>
        </div>
        <div class="oc-info">
          <h2>Orden de Compra</h2>
          <p><strong>${ocNum}</strong></p>
          <p>Fecha: ${new Date().toLocaleDateString('es-AR')}</p>
        </div>
      </div>
      <div class="proveedor">
        <strong>Proveedor:</strong> ${provActivo.nombre}
        ${provActivo.cuit ? `<br>CUIT: ${provActivo.cuit}` : ''}
        ${provActivo.direccion ? `<br>${provActivo.direccion}` : ''}
        ${provActivo.localidad ? `, ${provActivo.localidad}` : ''}
      </div>
      <table>
        <thead><tr><th>Código</th><th>Descripción</th><th class="text-right">Cant.</th><th class="text-right">P. Unit.</th><th class="text-right">Subtotal</th></tr></thead>
        <tbody>
          ${itemsList.map(i => `<tr><td>${i.codigo||'—'}</td><td>${i.desc}</td><td class="text-right">${i.qty}</td><td class="text-right">$${i.precio.toLocaleString('es-AR')}</td><td class="text-right">$${i.sub.toLocaleString('es-AR')}</td></tr>`).join('')}
        </tbody>
      </table>
      <div class="totals">
        <div class="row"><span>Subtotal:</span><span>$${ocTotal.toLocaleString('es-AR')}</span></div>
        <div class="row"><span>IVA 21%:</span><span>$${ocIva.toLocaleString('es-AR')}</span></div>
        <div class="row total"><span>TOTAL:</span><span>$${(ocTotal+ocIva).toLocaleString('es-AR')}</span></div>
      </div>
      <div class="footer">Kira Home - Bazar & Deco | kirahomebazar.com</div>
    </body></html>`
    ...
    setOcItems({}); setGeneratingPdf(false)
    if (provActivo) fetchDetalle(provActivo)
  }
  ...
  const serviciosAlerta = proveedores.filter(p => {
    ...
    const diff = (new Date(p.proximo_vencimiento).getTime() - new Date().setHours(0,0,0,0)) / (1000*60*60*24)
    return diff <= 7
  })
  ...
  if (vista === 'detalle' && provActivo) {
    const tabs: { id: Tab; label: string }[] = [
      ...
      ...(provActivo.tipo === 'mercaderia' ? [{ id: 'ordenes' as Tab, label: 'Orden de Compra' }] : []),
      { id: 'info', label: 'Datos' },
    ]
      ...
      <div className="p-4 lg:p-6 max-w-6xl mx-auto bg-botanical min-h-full">
        <div className="relative z-10">
            ...
            {tabs.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={cn("px-3 py-2 rounded-lg text-xs font-medium whitespace-nowrap transition-colors",
                  tab===t.id?"bg-kira-500 text-white":"bg-white border border-gray-200 text-gray-500 hover:bg-gray-50")}>{t.label}</button>
            ))}
          </div>
            ...
            <div className="space-y-4 animate-fade-in">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="bg-white rounded-xl px-4 py-3 border border-gray-100">
                  <p className="text-xs text-gray-400 uppercase tracking-wider">Deuda</p>
              ...
              {/* Selected comprobantes bar */}
              {selectedComps.size > 0 && (
                <div className="bg-kira-50 border border-kira-200 rounded-xl p-3 flex items-center justify-between animate-fade-in">
                  <span className="text-sm text-kira-800">{selectedComps.size} comprobante{selectedComps.size>1?'s':''} · Total pendiente: <strong>{formatCurrency(selectedTotal)}</strong></span>
                  <div className="flex gap-2">
                    <button onClick={() => setSelectedComps(new Set())} className="text-xs text-gray-500 hover:underline">Deseleccionar</button>
                    <button onClick={openPaySelected} className="px-3 py-1.5 text-xs bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 flex items-center gap-1"><Banknote size={12}/>Pagar selección</button>
                  </div>
                </div>
              )}
              ...
              {showOPForm && (
                <div className="bg-white rounded-xl border border-emerald-100 p-4 animate-fade-in">
                  <div className="flex justify-between items-center mb-3">
                    <h4 className="text-sm font-semibold text-gray-700">Nueva Orden de Pago</h4>
                    <button onClick={() => setShowOPForm(false)}><X size={14} className="text-gray-400"/></button>
                  </div>
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    <select value={opForm.metodo_pago} onChange={e => setOpForm(f=>({...f, metodo_pago: e.target.value as any}))}
                      className="px-3 py-2 text-sm border border-gray-200 rounded-lg"><option value="transferencia">Transferencia</option><option value="efectivo">Efectivo</option></select>
                  ...
                  {selectedComps.size > 0 && <p className="text-xs text-gray-400 mt-2">Afecta a {selectedComps.size} comprobante{selectedComps.size>1?'s':''} seleccionado{selectedComps.size>1?'s':''}</p>}
                  {selectedComps.size === 0 && <p className="text-xs text-amber-500 mt-2">Sin comprobantes seleccionados — se registrará como pago a cuenta</p>}
                </div>
              )}
              ...
              {showCompForm && (
                <div className="bg-white rounded-xl border border-kira-100 p-4 animate-fade-in">
                  <div className="flex justify-between items-center mb-3">
                    <h4 className="text-sm font-semibold text-gray-700">Nuevo Comprobante</h4>
                    <button onClick={() => setShowCompForm(false)}><X size={14} className="text-gray-400"/></button>
                  </div>
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <div>
                      <label className="text-[10px] text-gray-400">Emisión</label>
                      <input type="date" value={compForm.fecha} onChange={e => setCompForm(f=>({...f,fecha:e.target.value}))} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg" />
                    </div>
                    <div>
                      <label className="text-[10px] text-gray-400">Vencimiento</label>
                      <input type="date" value={compForm.fecha_vencimiento} onChange={e => setCompForm(f=>({...f,fecha_vencimiento:e.target.value}))} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg" />
                    </div>
                    ...
                    {COMPROBANTES_CON_IVA.includes(compForm.tipo_comprobante) && (<>
                      <input type="number" placeholder="IVA 21%" value={compForm.iva_21} onChange={e => setCompForm(f=>({...f,iva_21:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" />
                      <input type="number" placeholder="IVA 10.5%" value={compForm.iva_105} onChange={e => setCompForm(f=>({...f,iva_105:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" />
                      <input type="number" placeholder="Percepciones" value={compForm.percepciones} onChange={e => setCompForm(f=>({...f,percepciones:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" />
                    </>)}
              ...
              <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                ...
                <div className="divide-y divide-gray-50 max-h-[50vh] overflow-auto">
                  ...
                  ].sort((a,b) => new Date(b.date).getTime()-new Date(a.date).getTime()).map((mov) => {
                    if (mov.type==='comp') {
                      const c = mov.data as Comprobante; const esDeuda = COMPROBANTES_DEUDA.includes(c.tipo_comprobante)
                      const isPending = c.estado==='pendiente'||c.estado==='parcial'
                      return (
                        <div key={`c-${c.id}`} className={cn("flex items-center gap-3 px-4 py-3", selectedComps.has(c.id) && "bg-kira-50/50")}>
                          {isPending && (
                            <button onClick={() => toggleCompSelect(c.id)} className="flex-shrink-0">
                              {selectedComps.has(c.id) ? <CheckSquare size={16} className="text-kira-500"/> : <Square size={16} className="text-gray-300"/>}
                            </button>
                          )}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-400">
                              Em: {c.fecha}{c.fecha_vencimiento&&` · Vto: ${c.fecha_vencimiento}`}{c.descripcion&&` · ${c.descripcion}`}
                            </p>
                          </div>
                          <div className="text-right">
                            <p className={cn("text-sm font-semibold", esDeuda?"text-red-600":"text-emerald-600")}>{esDeuda?'+':'-'}{formatCurrency(c.total)}</p>
                            {c.saldo_pendiente>0 && c.saldo_pendiente<c.total && <p className="text-[10px] text-amber-500">Pend: {formatCurrency(c.saldo_pendiente)}</p>}
                      ...
                      return (
                        <div key={`p-${o.id}`} className="flex items-center gap-3 px-4 py-3">
                          <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 bg-blue-50 ml-6">
                  ...
                  {comprobantes.length===0 && ordenesPago.length===0 && <div className="px-4 py-8 text-center text-gray-400 text-sm">Sin movimientos</div>}
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4 animate-fade-in">
              ...
              {showCompForm && (
                <div className="bg-white rounded-xl border border-kira-100 p-4 animate-fade-in">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    <select value={compForm.tipo_comprobante} onChange={e => setCompForm(f=>({...f, tipo_comprobante: e.target.value as TipoComprobante}))} className="col-span-2 px-3 py-2 text-sm border border-gray-200 rounded-lg">
                      {Object.entries(COMPROBANTE_LABELS).map(([k,v]) => <option key={k} value={k}>{v}</option>)}
                    </select>
                    <input placeholder="Número" value={compForm.numero} onChange={e => setCompForm(f=>({...f,numero:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" />
                    ...
                    <input type="number" placeholder="Subtotal" value={compForm.subtotal} onChange={e => setCompForm(f=>({...f,subtotal:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" />
                    {COMPROBANTES_CON_IVA.includes(compForm.tipo_comprobante) && (<>
                      <input type="number" placeholder="IVA 21%" value={compForm.iva_21} onChange={e => setCompForm(f=>({...f,iva_21:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" />
                      <input type="number" placeholder="Percepciones" value={compForm.percepciones} onChange={e => setCompForm(f=>({...f,percepciones:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" />
                    </>)}
              ...
              <div className="bg-white rounded-xl border border-gray-100 divide-y divide-gray-50">
                {comprobantes.map(c => (
                  <div key={c.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-800">{COMPROBANTE_LABELS[c.tipo_comprobante]}{c.numero&&` #${c.numero}`}</p>
                      <p className="text-xs text-gray-400">Em: {c.fecha}{c.fecha_vencimiento&&` · Vto: ${c.fecha_vencimiento}`}{c.descripcion&&` · ${c.descripcion}`}</p>
                    </div>
            ...
            <div className="space-y-4 animate-fade-in">
              ...
              {showOPForm && (
                <div className="bg-white rounded-xl border border-emerald-100 p-4 animate-fade-in">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    <select value={opForm.metodo_pago} onChange={e => setOpForm(f=>({...f,metodo_pago:e.target.value as any}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg">
                      <option value="transferencia">Transferencia</option><option value="efectivo">Efectivo</option></select>
                  ...
                  <p className="text-xs text-amber-500 mt-2">Se registrará como pago a cuenta</p>
                </div>
              )}
              <div className="bg-white rounded-xl border border-gray-100 divide-y divide-gray-50">
                {ordenesPago.map(o => (
          ...
          {tab === 'ordenes' && provActivo.tipo === 'mercaderia' && (
            <div className="space-y-4 animate-fade-in">
              <div className="flex justify-between items-center">
                <h3 className="text-sm font-semibold text-gray-700">Generar Orden de Compra</h3>
                {Object.keys(ocItems).length > 0 && (
                  <button onClick={generarOCPdf} disabled={generatingPdf}
                    className="px-3 py-1.5 text-xs bg-kira-500 text-white rounded-lg hover:bg-kira-600 flex items-center gap-1.5 disabled:opacity-50">
                    <Download size={12}/>{generatingPdf ? 'Generando...' : `Cerrar OC — ${formatCurrency(ocTotal + ocIva)}`}
                  </button>
                )}
              </div>
              ...
              {/* OC summary */}
              {Object.keys(ocItems).length > 0 && (
                <div className="bg-kira-50 border border-kira-200 rounded-xl p-3 flex items-center justify-between">
                  <span className="text-sm text-kira-800">
                    {Object.keys(ocItems).length} artículo{Object.keys(ocItems).length>1?'s':''} · 
                    Subtotal: {formatCurrency(ocTotal)} · IVA: {formatCurrency(ocIva)} · <strong>Total: {formatCurrency(ocTotal + ocIva)}</strong>
                  </span>
                </div>
              )}
              ...
              <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {articulosProv.map(a => {
                      const qty = ocItems[a.id] || 0
                      return (
                        <tr key={a.id} className={cn(qty > 0 && "bg-kira-50/30")}>
                          ...
                          <td className="px-3 py-2 text-right">
                            <span className={cn("text-xs px-1.5 py-0.5 rounded font-semibold", a.cantidad<=0?"bg-red-100 text-red-700":a.cantidad<=2?"bg-amber-100 text-amber-700":"bg-emerald-100 text-emerald-700")}>{a.cantidad}</span>
                          </td>
                          <td className="px-3 py-2 text-right text-gray-600">{formatCurrency(a.costo_unitario)}</td>
                          <td className="px-3 py-2">
                            <div className="flex items-center justify-center gap-1.5">
                              {qty > 0 && (
                                <button onClick={() => setOcQty(a.id, qty-1)} className="w-7 h-7 rounded-md border border-gray-200 flex items-center justify-center hover:bg-gray-100 text-gray-500">−</button>
                              )}
                              {qty > 0 ? (
                                <input type="number" value={qty} onChange={e => setOcQty(a.id, parseInt(e.target.value)||0)}
                                  className="w-14 text-center px-1 py-1 text-sm border border-gray-200 rounded-md" />
                              ) : (
                                <button onClick={() => setOcQty(a.id, 1)} className="w-7 h-7 rounded-md border border-kira-200 bg-kira-50 flex items-center justify-center hover:bg-kira-100 text-kira-600">
                                  <Plus size={14} />
                                </button>
                              )}
                              {qty > 0 && (
                          ...
                          <td className="px-3 py-2 text-right font-semibold text-gray-800">{qty > 0 ? formatCurrency(a.costo_unitario * qty) : '—'}</td>
                        </tr>
                      )
                    })}
                    {articulosProv.length===0 && (
                      <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400 text-sm">Sin artículos vinculados a este proveedor</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
                ...
                {editingProv ? (
                  <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                    ...
                    {formProv.tipo==='servicio' && (<>
                      ...
                      <input type="date" value={formProv.proximo_vencimiento} onChange={e => setFormProv(f=>({...f,proximo_vencimiento:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" />
                    </>)}
      ...
      <div className="relative z-10">
        ...
        {serviciosAlerta.length > 0 && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4 animate-fade-in">
            ...
            {serviciosAlerta.map(p => (
              <div key={p.id} className="flex items-center justify-between text-sm"><span className="text-amber-800">{p.nombre}</span><span className="text-xs text-amber-600">{p.proximo_vencimiento}</span></div>
            ))}
          </div>
        )}
        ...
        {showGasto && (
          <div className="bg-white rounded-xl border border-amber-100 p-4 mb-4 animate-fade-in">
            <div className="flex justify-between items-center mb-3"><h3 className="text-sm font-semibold text-gray-700">Registrar Gasto</h3><button onClick={() => setShowGasto(false)}><X size={14} className="text-gray-400"/></button></div>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <input placeholder="Descripción *" value={gastoForm.descripcion} onChange={e => setGastoForm(f=>({...f,descripcion:e.target.value}))} className="col-span-2 px-3 py-2 text-sm border border-gray-200 rounded-lg" />
              <input type="number" placeholder="Monto *" value={gastoForm.monto} onChange={e => setGastoForm(f=>({...f,monto:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" />
              <select value={gastoForm.metodo_pago} onChange={e => setGastoForm(f=>({...f,metodo_pago:e.target.value as any}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg"><option value="efectivo">Efectivo</option><option value="transferencia">Transferencia</option></select>
              <input type="date" value={gastoForm.fecha} onChange={e => setGastoForm(f=>({...f,fecha:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" />
              <label className="flex items-center gap-2 text-sm text-gray-600"><input type="checkbox" checked={gastoForm.tiene_comprobante} onChange={e => setGastoForm(f=>({...f,tiene_comprobante:e.target.checked}))}/>Tiene comprobante</label>
              {gastoForm.tiene_comprobante && (<><input placeholder="Tipo" value={gastoForm.tipo_comprobante} onChange={e => setGastoForm(f=>({...f,tipo_comprobante:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" />
              <input placeholder="Nro" value={gastoForm.numero_comprobante} onChange={e => setGastoForm(f=>({...f,numero_comprobante:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" /></>)}
              <input placeholder="Nota" value={gastoForm.nota} onChange={e => setGastoForm(f=>({...f,nota:e.target.value}))} className="col-span-2 px-3 py-2 text-sm border border-gray-200 rounded-lg" />
              <button onClick={saveGasto} disabled={!gastoForm.descripcion||!gastoForm.monto} className="px-4 py-2 rounded-lg bg-amber-500 text-white text-sm font-medium hover:bg-amber-600 disabled:opacity-40 flex items-center justify-center gap-2"><Save size={14}/>Registrar</button>
            </div>
          </div>
        )}
        ...
        {showAdd && (
          <div className="bg-white rounded-xl border border-kira-100 p-4 mb-4 animate-fade-in">
            <div className="flex justify-between items-center mb-3"><h3 className="text-sm font-semibold text-gray-700">Nuevo Proveedor</h3><button onClick={() => setShowAdd(false)}><X size={14} className="text-gray-400"/></button></div>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <input placeholder="CUIT" value={formProv.cuit} onChange={e => setFormProv(f=>({...f,cuit:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" />
              {formProv.tipo==='servicio' && (<>
                <select value={formProv.frecuencia_pago} onChange={e => setFormProv(f=>({...f,frecuencia_pago:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg"><option value="">Frecuencia</option><option value="mensual">Mensual</option><option value="bimestral">Bimestral</option><option value="trimestral">Trimestral</option><option value="semestral">Semestral</option><option value="anual">Anual</option></select>
                <input type="date" value={formProv.proximo_vencimiento} onChange={e => setFormProv(f=>({...f,proximo_vencimiento:e.target.value}))} className="px-3 py-2 text-sm border border-gray-200 rounded-lg" />
              </>)}
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Buscar proveedor..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 rounded-lg border border-gray-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-kira-400/30 placeholder:text-gray-400" />
          </div>
          <div className="flex gap-2">
            {(['todos','mercaderia','servicio'] as const).map(f => (
              <button key={f} onClick={() => setFiltroTipo(f)} className={cn("px-3 py-2 rounded-lg text-xs font-medium transition-colors capitalize",
                filtroTipo===f?"bg-kira-500 text-white":"bg-white border border-gray-200 text-gray-500 hover:bg-gray-50")}>{f==='todos'?'Todos':f==='mercaderia'?'Mercadería':'Servicios'}</button>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 divide-y divide-gray-50">
          ...
          provsFiltrados.map(p => (
            <button key={p.id} onClick={() => openDetalle(p)} className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-kira-50/30 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 capitalize">{p.tipo}{p.cuit&&` · ${p.cuit}`}{p.tipo==='servicio'&&p.frecuencia_pago&&` · ${p.frecuencia_pago}`}
                  {p.tipo==='servicio'&&p.proximo_vencimiento&&(<span className={cn("ml-1",new Date(p.proximo_vencimiento)<=new Date()?"text-red-500 font-semibold":"")}> · Vence: {p.proximo_vencimiento}</span>)}
                </p>
              </div>
