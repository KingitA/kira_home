...
export default function VentasModule({ onVentaCompleta }: Props) {
  ...
  async function procesarVenta(){
    ...
    try{
      // Primary metodo_pago = the one with highest amount
  ...
  const mpIcons:Record<MetodoPago,any>={efectivo:<Banknote size={14} className="text-emerald-500"/>,transferencia:<CreditCard size={14} className="text-blue-500"/>,tarjeta_debito:<Wallet size={14} className="text-purple-500"/>,tarjeta_credito:<Wallet size={14} className="text-pink-500"/>}
  const mpColors:Record<MetodoPago,string>={efectivo:'focus:ring-emerald-400/30 focus:border-emerald-400',transferencia:'focus:ring-blue-400/30 focus:border-blue-400',tarjeta_debito:'focus:ring-purple-400/30 focus:border-purple-400',tarjeta_credito:'focus:ring-pink-400/30 focus:border-pink-400'}
          ...
          <div className="border-t border-gray-100 px-4 py-4 space-y-3 max-h-[55vh] overflow-auto">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                {pagos.length<4&&<button onClick={addPago} className="text-[10px] text-kira-600 hover:underline flex items-center gap-0.5"><Plus size={10}/>Agregar método</button>}
              </div>
              <div className="space-y-2">
                {pagos.map((p,idx)=>(
                  <div key={idx} className="space-y-1.5">
                    <div className="flex items-center gap-2">
                      <select value={p.metodo} onChange={e=>updatePago(idx,'metodo',e.target.value)}
                        className="w-36 px-2 py-2 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-kira-400/30">
                        ...
                        {[1,3,6,12].map(c=>(
                          <button key={c} onClick={()=>updatePago(idx,'cuotas',c)}
                            className={cn("px-2 py-0.5 rounded text-[10px] font-medium transition-colors",
                              p.cuotas===c?"bg-pink-100 text-pink-700 border border-pink-200":"bg-gray-50 text-gray-500 border border-gray-200 hover:bg-gray-100")}>
                            {c}
                          </button>
                        ))}
                      </div>
