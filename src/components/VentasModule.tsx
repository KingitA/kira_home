...
const IllBano = () => (<svg viewBox="0 0 120 80" className="w-full h-full" fill="none"><ellipse cx="60" cy="72" rx="50" ry="4" fill="currentColor" opacity="0.1"/><rect x="25" y="35" width="20" height="30" rx="4" fill="currentColor" opacity="0.15" stroke="currentColor" strokeWidth="1.5"/><rect x="32" y="25" width="6" height="12" rx="2" fill="currentColor" opacity="0.2"/><path d="M38 28L45 28L45 32" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" fill="none" opacity="0.3"/><rect x="60" y="28" width="30" height="38" rx="3" fill="currentColor" opacity="0.1" stroke="currentColor" strokeWidth="1.5"/></svg>)
...
export default function VentasModule({ onVentaCompleta }: Props) {
  ...
  function addToCart(a:Articulo) {
    setCarrito(prev=>{const ex=prev.find(i=>i.articulo.id===a.id);if(ex){if(ex.cantidad>=a.cantidad){setError(`Stock máx: ${a.cantidad}`);setTimeout(()=>setError(''),2500);return prev}return prev.map(i=>i.articulo.id===a.id?{...i,cantidad:i.cantidad+1}:i)}return[...prev,{articulo:a,cantidad:1}]})
    setSearch(''); if(!categoriaActiva) setResults([])
  }
  ...
  const clearCart=()=>{setCarrito([]);setMontoEfectivo('');setMontoTransferencia('');setError('');setDescuentoContado(false)}
  ...
  async function procesarVenta() {
    ...
    try {
      ...
      setVentaExitosa(true);setCarrito([]);setMontoEfectivo('');setMontoTransferencia('');setDescuentoContado(false);fetchArticulos();onVentaCompleta()
        ...
        <div className="max-w-3xl mx-auto">
          ...
          {!categoriaActiva&&search.length<2&&(
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-6 animate-fade-in">
              {CATEGORIAS.map(cat=>(<button key={cat.id} onClick={()=>setCategoriaActiva(cat.id)} className={`group relative overflow-hidden rounded-xl border h-36 lg:h-40 text-left transition-all hover:scale-[1.02] hover:shadow-md active:scale-[0.98] ${cat.bg}`}><div className={`absolute top-2 right-2 w-20 h-20 lg:w-24 lg:h-24 ${cat.accent} opacity-60 group-hover:opacity-80 transition-opacity`}><cat.Ill/></div><div className="absolute inset-0 flex flex-col justify-end p-3.5"><h3 className={`font-semibold text-sm lg:text-base ${cat.text}`}>{cat.nombre}</h3><p className={`text-[11px] ${cat.accent} opacity-70`}>{countCat(cat)} productos</p></div></button>))}
              <button onClick={()=>setCategoriaActiva('todos')} className="group relative overflow-hidden rounded-xl border border-gray-200 h-36 lg:h-40 text-left transition-all hover:scale-[1.02] hover:shadow-md bg-white"><div className="absolute top-3 right-3 text-gray-300 opacity-40"><Package size={48}/></div><div className="absolute inset-0 flex flex-col justify-end p-3.5"><h3 className="font-semibold text-sm lg:text-base text-gray-700">Ver Todo</h3><p className="text-[11px] text-gray-400">{articulos.length} productos</p></div></button>
            </div>
          )}
          {results.length>0&&(<div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden animate-fade-in"><div className="max-h-[55vh] overflow-auto divide-y divide-gray-50">{results.map(a=>{const inCart=carrito.find(i=>i.articulo.id===a.id);const rem=a.cantidad-(inCart?.cantidad??0);return(<button key={a.id} onClick={()=>addToCart(a)} disabled={rem<=0} className={cn("w-full flex items-center gap-3 px-4 py-3 text-left transition-colors",rem<=0?"opacity-40 cursor-not-allowed":"hover:bg-kira-50/50")}><div className="w-8 h-8 rounded-lg bg-kira-100 flex items-center justify-center flex-shrink-0"><Package size={14} className="text-kira-600"/></div><div className="flex-1 min-w-0"><p className="text-sm font-medium text-gray-800 truncate">{a.descripcion}</p><p className="text-xs text-gray-400">{a.codigo&&`${a.codigo} · `}{a.proveedor}{inCart&&<span className="text-kira-500 ml-1">· {inCart.cantidad} en carrito</span>}</p></div><div className="text-right flex-shrink-0"><p className="text-sm font-semibold text-gray-800">{formatCurrency(a.precio_venta)}</p><p className={cn("text-[10px]",rem<=2?"text-amber-500":"text-gray-400")}>Stock: {rem}</p></div><Plus size={16} className="text-gray-300 flex-shrink-0"/></button>)})}</div></div>)}
        ...
        {carrito.length>0&&(
          <div className="border-t border-gray-100 px-4 py-4 space-y-3">
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-500">Subtotal ({totalItems} art.)</span>
              <span className="text-sm text-gray-700">{formatCurrency(subtotal)}</span>
            </div>

            {/* Descuento contado */}
            <label className="flex items-center gap-2 cursor-pointer group">
              <input type="checkbox" checked={descuentoContado} onChange={e=>{setDescuentoContado(e.target.checked);if(e.target.checked){const t=subtotal*0.9;setMontoEfectivo(t.toString());setMontoTransferencia('')}}}
                className="w-4 h-4 rounded border-gray-300 text-kira-500 focus:ring-kira-400"/>
              <Percent size={13} className="text-kira-500"/>
              <span className="text-xs text-gray-600 group-hover:text-gray-800">Descuento contado 10%</span>
              {descuentoContado&&<span className="text-xs font-semibold text-emerald-600 ml-auto">-{formatCurrency(descuentoMonto)}</span>}
            </label>
            ...
            <div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative flex-1"><span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-gray-400">$</span>
                    <input type="number" placeholder="0" value={montoEfectivo} onChange={e=>setMontoEfectivo(e.target.value)} className="w-full pl-6 pr-3 py-2 rounded-lg border border-gray-200 text-sm text-right focus:outline-none focus:ring-2 focus:ring-emerald-400/30 focus:border-emerald-400"/></div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative flex-1"><span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-gray-400">$</span>
                    <input type="number" placeholder="0" value={montoTransferencia} onChange={e=>setMontoTransferencia(e.target.value)} className="w-full pl-6 pr-3 py-2 rounded-lg border border-gray-200 text-sm text-right focus:outline-none focus:ring-2 focus:ring-blue-400/30 focus:border-blue-400"/></div>
              ...
              {totalPagado>0&&ajuste!==0&&(
                <div className={cn("flex items-center justify-between mt-2 px-2 py-1.5 rounded-lg text-xs",ajuste>0?"bg-amber-50 text-amber-700":"bg-red-50 text-red-600")}>
                  <span>{ajuste>0?'Vuelto / Ajuste a favor':'Falta cobrar'}</span><span className="font-semibold">{formatCurrency(Math.abs(ajuste))}</span>
                </div>
              )}
            </div>
